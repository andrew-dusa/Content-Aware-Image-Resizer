...
int main() {
  ...
  try{
  if(static_cast<unsigned int>(targetWidth) > width || static_cast<unsigned int>(targetHeight) > height || 
  targetWidth <= 0 || targetHeight <= 0 || 
  static_cast<unsigned int>(targetWidth) >= MAX_WIDTH || static_cast<unsigned int>(targetHeight) >= MAX_HEIGHT){
